<template>
  <el-main class="el-main">
    ...
    <el-card class="formCard">
      <el-form :model="answerForm" :rules="formRules" ref="answerForm" label-width="250px" class="inputForm"
               label-position="left">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      // },
      formRules: {
        price: [{required: true, validator: fixedNumberRule, trigger: "blur"}],
        fixedCost: [{required: true, validator: fixedNumberRule, trigger: "blur"}],
        variableCost: [{required: true, validator: fixedNumberRule, trigger: "blur"}],
  ...
  methods: {
    setQuestion(){
      this.question="某鼠标生产商去年一年销售某型号鼠标共2万只，总销售额为200万元，总成本为145万元，其中固定成本为45万。今年该公司计划增加广告费5万元，同时花费15万元改进了产品的生产线，生产同样一只鼠标较原来能节省5元。但由于国际金属的价格波动，每只鼠标的生产成本上升了2元。"
    },
    submit(){
