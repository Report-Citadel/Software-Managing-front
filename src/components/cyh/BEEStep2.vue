<template>
  <el-main style="width: 800px">
    ...
    <el-form :model="answerForm" :rules="formRules" ref="bEPForm" label-width="250px" class="inputForm"
             label-position="left">
    ...
    <el-row>
      ...
      <el-col :span="6">
        <el-button icon="el-icon-arrow-left" @click="previousStep">上一页</el-button>
      </el-col>
      <el-col :span="3">&nbsp;</el-col>
      <el-col :span="6">
        <el-button type="primary" icon="el-icon-check" @click="nextStep('bEPForm')">确认</el-button>
      </el-col>
    </el-row>
  </el-main>
</template>
...
export default {
  name: "BEEStep2",
  data(){
    const fixedNumberRule = (rule, value, callback) => {//包含小数的数字
      let reg = /^[+-]?(0|([1-9]\d*))(\.\d+)?$/g;
      if (value === '') {
    ...
    return{
      ...
      formRules: {
        ...
        jobRate: [{required: true, validator: fixedNumberRule, trigger: "blur"}],
        safeRate: [{required: true, validator: fixedNumberRule, trigger: "blur"}],
      }
    }
  },
  props:[
  ...
  methods:{
    setVariableCosts(){
      for(let i=0;i<this.xAxisData.length;i++){
        this.variableCosts.push(this.xAxisData[i]*Number(this.answerForm.variableCost));
      }
    },
    setTotalCosts(){
      for(let i=0;i<this.xAxisData.length;i++){
        this.totalCosts.push(this.xAxisData[i]*Number(this.answerForm.variableCost)+Number(this.answerForm.fixedCost));
      }
    },
    setSales(){
      for(let i=0;i<this.xAxisData.length;i++){
